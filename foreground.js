...
//TODO: change query selector to xpath || MERGE THIS FUNCTON WITH CHATBODYCONTAINER DETETOR FUNCTION
/***********************************
...
const observer = new MutationObserver((mutationsList, observer) => {
  //chat body container
  const chatBodyContainerClasses = 'flex flex-col text-sm dark:bg-gray-800';
  const chatBodyContainerXPath = '//*[@id="__next"]/div[2]/div[2]/div/main/div[2]/div/div/div';
  // Evaluate the XPath expression on the document
  const result = document.evaluate(chatBodyContainerXPath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);

  // Check if an element is found
  if (result.singleNodeValue) {
    console.log('Element exists!');
    let chatBodyContainer = result.singleNodeValue;
    // Disconnect the observer once the element is rendered
    observer.disconnect();
    console.log(chatBodyContainer.childNodes.length)
  }
});
